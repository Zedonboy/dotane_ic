...
use ic_stable_structures::{memory_manager::VirtualMemory, storable::Bound, DefaultMemoryImpl, Memory, StableBTreeMap, Storable};
use num_traits::ToPrimitive;
use serde::Serialize;
use serde_bytes::ByteBuf;
use sha2::Digest;
use std::{borrow::Cow, collections::{BTreeSet, HashMap}, cell::RefCell};
...
pub struct PathEncodingKey(pub String, pub String);

impl Storable for PathEncodingKey {
...
pub struct RcBytesVec(pub Vec<RcBytes>);

impl Storable for RcBytesVec {
    fn to_bytes(&self) -> Cow<[u8]> {
        Cow::Owned(candid::encode_one(self).unwrap())
    }
...
thread_local! {
    static ASSET_CACHE: RefCell<Option<StableBTreeMap<PathEncodingKey, RcBytesVec, VMemory>>> = RefCell::new(None);
}

/// Initialize the ASSET_CACHE with a custom Memory implementation
pub fn init_asset_cache(memory: VMemory) {

    ASSET_CACHE.set(Some(StableBTreeMap::init(memory)));
}
...
impl AssetEncoding {
    pub fn get_content_chunks(&self, path: &str, encoding: &str) -> Vec<RcBytes> {
        ASSET_CACHE.with_borrow(|cache| {
            cache
                .as_ref()
                .and_then(|map| map.get(&PathEncodingKey(path.to_string(), encoding.to_string())))
                .map(|rc_bytes_vec| rc_bytes_vec.0)
                .unwrap_or_default()
        })
    }

    pub fn set_content_chunks(&self, path: &str, encoding: &str, chunks: Vec<RcBytes>) {
        ASSET_CACHE.with_borrow_mut(|cache| {
            if let Some(map) = cache.as_mut() {
                map.insert(
                    PathEncodingKey(path.to_string(), encoding.to_string()),
                    RcBytesVec(chunks),
                );
            }
        });
    ...
    pub fn remove_content_chunks(&self, path: &str, encoding: &str) {
        ASSET_CACHE.with_borrow_mut(|cache| {
            if let Some(map) = cache.as_mut() {
                map.remove(&PathEncodingKey(path.to_string(), encoding.to_string()));
            }
        });
    }
...
#[derive(Clone, Debug, CandidType, Deserialize, Default, Serialize)]
...
impl State {
    ...
    pub fn set_asset_content(
        ...
        // Store content_chunks in ASSET_CACHE
        enc.set_content_chunks(&arg.key, &arg.content_encoding, content_chunks);
    ...
    pub fn unset_asset_content(&mut self, arg: UnsetAssetContentArguments) -> Result<(), String> {
        ...
        if asset.encodings.remove(&arg.content_encoding).is_some() {
            // Clean up content_chunks from ASSET_CACHE
            ASSET_CACHE.with_borrow_mut(|cache| {
                if let Some(map) = cache.as_mut() {
                    map.remove(&PathEncodingKey(arg.key.clone(), arg.content_encoding.clone()));
                }
            });
    ...
    pub fn delete_asset(&mut self, arg: DeleteAssetArguments) {
        if let Some(asset) = self.assets.get(&arg.key) {
            ...
            for encoding_name in asset.encodings.keys() {
                ASSET_CACHE.with_borrow_mut(|cache| {
                    if let Some(map) = cache.as_mut() {
                        map.remove(&PathEncodingKey(arg.key.clone(), encoding_name.clone()));
                    }
                });
            }
    ...
    pub fn retrieve(&self, key: &AssetKey) -> Result<RcBytes, String> {
        ...
        let content_chunks = id_enc.get_content_chunks(key, "identity");
        if content_chunks.len() > 1 {
            return Err("Asset too large. Use get() and get_chunk() instead.".to_string());
        }

        content_chunks.into_iter().next()
            .ok_or_else(|| "no content chunks found".to_string())
    }

    pub fn store(&mut self, arg: StoreArg, time: u64) -> Result<(), String> {
        debug_print(&format!("storing asset 2"));
        let dependent_keys = self.dependent_keys(&arg.key);
        ...
        let encoding = asset.encodings.entry(arg.content_encoding.clone()).or_default();
        encoding.total_length = arg.content.len();
        encoding.set_content_chunks(&arg.key, &arg.content_encoding, vec![RcBytes::from(arg.content)]);
        encoding.modified = Int::from(time);
        encoding.sha256 = hash;

        on_asset_change(&mut self.asset_hashes, &arg.key, asset, dependent_keys);
        debug_print(&format!("asset stored"));
        Ok(())
    }

    /// Save an asset directly during runtime with the specified path and content
    pub fn save_asset_runtime(
        &mut self,
        path: String,
        content: Vec<u8>,
        content_type: String,
        ...
        let arg = StoreArg {
            ...
            sha256: None, // Let it compute the hash automatically
            aliased: None,
        };
    ...
    /// Save an asset directly during runtime with automatic timestamp
    pub fn save_asset_now(
        &mut self,
        path: String,
        content: Vec<u8>,
        content_type: String,
        ...
        self.save_asset_runtime(path, content, content_type, content_encoding, headers, time)
    }
    ...
    pub fn get(&self, arg: GetArg) -> Result<EncodedAsset, String> {
        ...
        for enc in arg.accept_encodings.iter() {
            if let Some(asset_enc) = asset.encodings.get(enc) {
                let content_chunks = asset_enc.get_content_chunks(&arg.key, enc);
                let content = content_chunks.into_iter().next()
                    .ok_or_else(|| "no content chunks found".to_string())?;
    ...
    pub fn get_chunk(&self, arg: GetChunkArg) -> Result<RcBytes, String> {
        ...
        let content_chunks = enc.get_content_chunks(&arg.key, &arg.content_encoding);
        if arg.index >= content_chunks.len() {
            return Err("chunk index out of bounds".to_string());
        }
        let index: usize = arg.index.0.to_usize().unwrap();

        Ok(content_chunks[index].clone())
    }
    ...
    fn build_http_response(
        ...
        if let Ok(asset) = self.get_asset(&path.into()) {
            if !asset.allow_raw_access() && req.is_raw_domain() {
                return Ok(req.redirect_from_raw_to_certified_domain());
            }
        }
        ...
        if witness_result == WitnessResult::FallbackFound {
            if let Ok(asset) = self.get_asset(&FALLBACK_FILE.to_string()) {
                debug_print(&format!("fallback asset"));
        ...
        if self.get_asset(&path.into()).is_err() {
            return Err(format!("Asset not found: {}", path));
        }

        Ok(HttpResponse::build_404(certificate_header, req.get_certificate_version()))
    }
    ...
    pub fn http_request_streaming_callback(
        ...
        let content_chunks = enc.get_content_chunks(&key, &content_encoding);

        Ok(StreamingCallbackHttpResponse {
            body: content_chunks[chunk_index].clone(),
    ...
    fn dependent_keys(&self, _key: &AssetKey) -> Vec<AssetKey> {
        Vec::new()
    }
...
impl Storable for StableState {
    ...
    fn from_bytes(bytes: std::borrow::Cow<[u8]>) -> Self {
        let result = serde_cbor::from_slice::<StableState>(&bytes);
        if bytes.len() == 0 {
            return StableState::default();
        }
        if result.is_err() {
            return StableState::default();
        } else {
