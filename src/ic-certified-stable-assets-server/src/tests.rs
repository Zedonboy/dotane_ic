...
pub fn verify_response(
    ...
    *header_value = header_value.replace(
    ...
    let request = ic_http_certification::http::HttpRequestBuilder::new()
        .with_method(if request.method == "GET" {
            ic_http_certification::http::Method::GET
        } else if request.method == "POST" {
            ic_http_certification::http::Method::POST
        } else {
            panic!("Unsupported method: {}", request.method);
        })
    ...
    // };
    let response = ic_http_certification::http::HttpResponseBuilder::new()
...
fn certified_http_request(state: &State, request: HttpRequest) -> HttpResponse {
    let response = state.http_request(request.clone(), &[], unused_callback()).unwrap();
...
fn support_aliases() {
    ...
    let empty_path_alias =
        state.http_request(RequestBuilder::get("").build(), &[], unused_callback()).unwrap();
