...
pub const FALLBACK_FILE: &str = "/404";
...
impl HttpResponse {
    ...
    pub fn build_ok(
        ...
        debug_print(&format!("enc_name: {:?}", enc_name));
        debug_print(&format!("enc: {:?}", enc));
        debug_print(&format!("key: {:?}", key));
        let content_chunks = enc.get_content_chunks(key, enc_name);
        debug_print(&format!("content_chunks length: {:?}", content_chunks.len()));
        ...
        let (status_code, body) = if etags.contains(&enc.sha256) {
            ...
            (200, content_chunks[chunk_index].clone())
        };
